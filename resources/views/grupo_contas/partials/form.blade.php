    ...
    <div class="card-body">
        ...
        <div class="form-group">
            ...
            <input type="text" name="sigla" class="form-control" required
                value="{{ old('sigla', $grupoConta->sigla ?? '') }}">
        </div>

        <div class="form-group">
            <label for="ordem">Ordem*</label>
            <input type="text" name="ordem" class="form-control" required
                value="{{ old('ordem', $grupoConta->ordem ?? '') }}">
        </div>
        ...
        <div class="form-group">
            ...
            <select name="nivel" class="form-control" required>
                @for($i = 1; $i <= 5; $i++)
                    <option value="{{ $i }}" {{ old('nivel', $grupoConta->nivel ?? '') == $i ? 'selected' : '' }}>{{ $i }}
                    </option>
                @endfor
            </select>
        </div>
        ...
        <div class="form-group">
            ...
            <select name="operacao" class="form-control" required>
                <option value="(+)" {{ old('operacao', $grupoConta->operacao ?? '') == 'SOMA' ? 'selected' : '' }}>(+)
                    Soma
                </option>
        ...
        <div class="form-check">
            <input class="form-check-input" type="checkbox" name="apresentar_relatorios" id="apresentar_relatorios"
                value="1" {{ old('apresentar_relatorios', $grupoConta->apresentar_relatorios ?? false) ? 'checked' : '' }}>
