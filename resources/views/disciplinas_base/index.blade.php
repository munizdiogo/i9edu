...
@section('title', 'Disciplinas Base')
@section('content_header')
    <h1 class="d-inline">Disciplinas Base</h1>
    <a href="{{ route('disciplinas_base.create') }}" class="btn btn-success float-right">Nova Disciplina</a>
@endsection
@section('css')
...
@section('js')
    ...
    <script>
        $(function () {
            $('#disciplinas-base-table').DataTable({
                processing: true,
                serverSide: true,
                ajax: '{{ route("disciplinas_base.data") }}',
                columns: [
                    { data: 'id' },
                    { data: 'codigo' },
                    { data: 'nome' },
