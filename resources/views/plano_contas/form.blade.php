...
<div class="mb-3">
    ...
    <input type="text" class="form-control" name="codigo" value="{{ old('codigo', $planoConta->codigo ?? '') }}"
        required>
</div>
<div class="mb-3">
    <label for="operacao" class="form-label">Operação*</label>
    <select name="operacao" class="form-select" required>
        <option value="Soma" {{ old('operacao', $planoConta->operacao ?? '') == 'Soma' ? 'selected' : '' }}>Soma
        </option>
...
<div class="mb-3">
    ...
    <select name="status" class="form-select" required>
        <option value="Ativo" {{ old('status', $planoConta->status ?? '') == 'Ativo' ? 'selected' : '' }}>Ativo
        </option>
        <option value="Inativo" {{ old('status', $planoConta->status ?? '') == 'Inativo' ? 'selected' : '' }}>Inativo
        </option>
    </select>
</div>
<div class="mb-3">
...
<div class="mb-3">
    ...
    <select name="natureza" class="form-select" required>
        <option value="ATIVO" {{ old('natureza', $planoConta->natureza ?? '') == 'ATIVO' ? 'selected' : '' }}>ATIVO
        </option>
