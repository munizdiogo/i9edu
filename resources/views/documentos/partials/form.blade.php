<div class="card card-primary">
    <div class="card-body">
        ...
        <div class="row">
            <div class="col-md-6">
                ...
                <select name="status" class="form-control" required>
                    <option value="Ativo" {{ old('status', $documento->status ?? '') == 'Ativo' ? 'selected' : '' }}>Ativo
                    </option>
                    <option value="Inativo" {{ old('status', $documento->status ?? '') == 'Inativo' ? 'selected' : '' }}>
                        Inativo
                    </option>
                </select>
            </div>
            <div class="col-md-6">
                ...
                <select name="tipo" class="form-control" required>
                    <option value="Matrícula" {{ old('tipo', $documento->tipo ?? '') == 'Matrícula' ? 'selected' : '' }}>
                        Matrícula
                    </option>
                    <option value="Contrato" {{ old('tipo', $documento->tipo ?? '') == 'Contrato' ? 'selected' : '' }}>
                        Contrato
                    </option>
                    <option value="Outro" {{ old('tipo', $documento->tipo ?? '') == 'Outro' ? 'selected' : '' }}>Outro
                    </option>
                </select>
            </div>
        </div>
        ...
        <div class="form-group mt-2">
            ...
            <div class="form-check">
                <input type="checkbox" name="obrigatorio_informar_data" class="form-check-input"
                    id="obrigatorio_informar_data" {{ old('obrigatorio_informar_data', $documento->obrigatorio_informar_data ?? false) ? 'checked' : '' }}>
